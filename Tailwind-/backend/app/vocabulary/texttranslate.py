...
# 配置
# ----------------------------
input_file = Path("Tailwind-/backend/app/vocabulary/Tailwind-/backend/app/vocabulary/words_only.txt")
output_file = input_file.parent / "words_translated.txt"
...
async def translate_all(words, batch_size=BATCH_SIZE, concurrent=CONCURRENT):
    ...
    async with aiohttp.ClientSession() as session:
        for i in range(0, len(words), batch_size * concurrent):
            tasks = []
            chunks = [words[i+j:i+j+batch_size] for j in range(0, batch_size*concurrent, batch_size)]
            for chunk in chunks:
            ...
            with open(output_file, "a", encoding="utf-8") as f:
                for chunk_words, chunk_translations in zip(chunks, results):
                    for w, t in zip(chunk_words, chunk_translations):
                        ...
                        print(f"\r✅ 已翻译 {translated_count_local}/{total} 行 "
                              f"({percent:.2f}%) - ETA: {eta:.1f}s - Speed: {speed:.2f} 行/s", end="")
